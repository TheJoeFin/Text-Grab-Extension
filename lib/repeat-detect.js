...
(() => {
  ...
  function pickRecordSetInRegion(sets, region) {
    ...
    for (const set of sets) {
      const items = set.items.filter((el) => regionOverlap(el.getBoundingClientRect(), region) > 0);
      if (items.length < 2) continue; // need at least two records to be a table
      let area = 0;
      for (const el of items) area += regionOverlap(el.getBoundingClientRect(), region);
      if (area > bestArea) {
        bestArea = area;
        best = set;
        bestItems = items;
  ...
  /** Overlap area (px²) between a DOMRect and a viewport region rect. */
