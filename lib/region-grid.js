...
(() => {
  ...
  function collectFragments(region) {
    ...
    for (let node = walker.nextNode(); node; node = walker.nextNode()) {
      ...
      const rects = [...range.getClientRects()].filter(
        (r) => (r.width || r.height) && intersects(r, region)
      );
      if (rects.length === 0) continue;

      const text = clipText(node, region);
      if (!text) continue;
  ...
  /** Words of a text node whose rendered rects fall inside the region. */
  function clipText(node, region) {
    ...
    while ((match = wordPattern.exec(text)) !== null) {
      ...
      if (intersects(range.getBoundingClientRect(), region)) kept.push(match[0]);
    }
    return kept.join(' ');
  }
