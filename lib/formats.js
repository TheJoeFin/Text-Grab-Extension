...
(() => {
   ...
   * natively) but dropping classes, styles, and non-text content. Works for
   * native <table>s and ARIA grids alike — both are walked with the shared
   * tableGrid row/cell helpers, which read colspan/rowspan from the DOM API or
   * aria-colspan/aria-rowspan respectively. Falls back to serializing the
   * already-expanded grid (merges flattened to blanks) when no element is
   * given, e.g. a region-clipped capture or a reconstructed list.
   * @param {Element|null} table
   * @param {string[][]} grid
   */
  function toCleanHtmlTable(table, grid) {
    const tg = TG.tableGrid;
    if (!table || !table.matches?.(tg.TABLE_SELECTOR)) return gridToHtmlTable(grid);

    const { cellText, collectRows, collectCells, getColSpan, getRowSpan } = tg;
    const rows = [];
    for (const row of collectRows(table)) {
      const cells = [];
      for (const cell of collectCells(row, table)) {
        const role = cell.getAttribute('role');
        const isHeader =
          cell.tagName === 'TH' || role === 'columnheader' || role === 'rowheader';
